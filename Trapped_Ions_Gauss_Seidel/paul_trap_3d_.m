...
%% Ausgangsparameter
typer='paul_classic';    %Typ des Systems
sizer=128;              %Auflösung 
iter_steps=8192;       %Iterationsschritte (in der Zeit)
dt=0.04;
omega=2;                %Frequenz der Wechselspannung
q=-0.000008;                %Ladung der Teilchen
m=0.00008;                  %Masse der Teilchen 
visc=0.25;
...
%Lorentzfit
lorentz=2;              %1 - Gradient des Potentials; 2 - Ideales Potential

plot_rec_step=200;
ausgangsplot=1;
trajectories=2500;         %Zeige Trajektorien?
...
for i=1:iter_steps      %Iterationsschritte
    ...
    if lorentz==1
        %Numerically calculated potential
        for j=1:n_q
    ...
    elseif lorentz==2
        ...
        for j=1:n_q
            if x_i(j)>0 && x_i(j)<1 && y_i(j)>0 && y_i(j)<1 && z_i(j)>0 && z_i(j)<1
                ...
                vx(j,i+1)=vx(j,i)*(1-visc*dt)+q/m*dt*(-const*a*(nx(j,i)-0.5)*fun(omega,t)+E_q_x);
                vy(j,i+1)=vy(j,i)*(1-visc*dt)+q/m*dt*(-const*b*(ny(j,i)-0.5)*fun(omega,t)+E_q_y);
                vz(j,i+1)=vz(j,i)*(1-visc*dt)+q/m*dt*(-const*c*(nz(j,i)-0.5)*fun(omega,t)+E_q_z)-g_const*dt;
                nx(j,i+1)=nx(j,i)+vx(j,i)*dt;
                ny(j,i+1)=ny(j,i)+vy(j,i)*dt;
                nz(j,i+1)=nz(j,i)+vz(j,i)*dt;
            else
