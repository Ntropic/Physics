...
n_max=10;           %How Many Fourier Components?
how_fine=15;        %How much Detail per Zone?
plot_how_many=7;   %Plot How many Bands?
plot_light_cone=1;
plotting=[1,1];     %E,B

%Save Startparameters
appdata_save(f,'d',d,'a',a,'eps_1',eps_1,'eps_2',eps_2,'n_max',n_max,'how_fine',how_fine,'plot_before',0);
...
freqs_TE=zeros(length(k_x),pw);
freqs_TM=zeros(length(k_x),2*pw);
...
for i=1:length(k_x)
    ...
    freqs_TE(i,:)=omega_k_TE;
    freqs_TM(i,:)=omega_k_TM;
...
if any(plotting)==1
    ...
    s=1;
        ...
        while counter_E<=plot_how_many || counter_M<=plot_how_many 
            if plotting(1)==1
                if counter_E<=plot_how_many
                    if max(real(freqs_TE(:,s)))>1e-5
                        plot(1:length(k_x),real(freqs_TE(:,s)),'b')
            ...
            if plotting(2)==1
                if counter_M<=plot_how_many
                    if max(real(freqs_TM(:,s)))>1e-5
                        plot(1:length(k_x),real(freqs_TM(:,s)),'r')
    ...
    if plotting(1)==1
        for t=s_E+1:s_E+5
            if max(real(freqs_TE(:,t)))>1e-5
                plot(1:length(k_x),real(freqs_TE(:,t)),'b')
            end
        end
    end
    if plotting(2)==1
        for t=s_M+1:s_M+5
            if max(real(freqs_TM(:,t)))>1e-5
                plot(1:length(k_x),real(freqs_TM(:,t)),'r')
            end
        end
    end
