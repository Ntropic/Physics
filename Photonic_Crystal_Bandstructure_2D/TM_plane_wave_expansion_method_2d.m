...
filename=filename_generator('Surf_TM',d,a,eps_1,eps_2,n_max,how_fine,plot_how_many);
...
while counter<=plot_how_many 
    if max(real(freqs(:,s)))>1e-5
        plot(1:length(k_x),real(freqs(:,s)),'ButtonDownFcn',{@MouseMovement_2D_TEM_Toggle,f,h,'TM'})
        counter=counter+1;
        if counter==2
            hold on;
        end
...
set(f,'ButtonDownFcn',{@MouseMovement_2D_TEM_Toggle,f,h,'TM'});
