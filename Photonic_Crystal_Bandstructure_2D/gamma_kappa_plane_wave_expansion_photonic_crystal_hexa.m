...
%Gamma-Kappa-M-Gamma -> use high symmetry points
clc;
clear all;
close all;
...
how_fine=10;        %How much Detail per Zone?
plot_how_many=14;   %Plot How many Bands?
plot_light_cone=0;
...
while counter<=plot_how_many 
    if max(real(freqs(:,s)))>1e-5
        plot(1:length(k_x),real(freqs(:,s)))
        counter=counter+1;
        if counter==2
            hold on;
        end
